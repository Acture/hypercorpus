...
KEY_COLS = [
    "dataset", "selector", "selector_family", "selector_provider",
    "selector_model", "seed_strategy", "seed_top_k", "hop_budget",
    "baseline", "search_structure", "edge_scorer", "lookahead_depth",
    "profile_name", "budget_fill_mode", "budget_mode", "budget_value",
    "budget_label", "token_budget_tokens",
]
...
def read_summary_rows(run_dir: Path) -> list[dict]:
    ...
    for key, chunk_rows in groups.items():
        ...
        print(f"    Aggregated {len(chunk_rows)} chunks, n={total_n}: "
...
def extract_table3():
    ...
    ablation_rows = read_summary_rows(
        RUNS / "2wiki-single-path-edge-ablation-s100-v1"
    )
    baseline_rows = read_summary_rows(
        RUNS / "2wiki-baseline-retest-s100-v1"
    )

    # Define which selectors go into Table 3 and their display names
    table_spec = [
        # (source, match_fn, display_name)
        ("ablation", lambda r: r.get("selector_family") == "diagnostic"
         and "gold_support" in r.get("selector", ""), "Oracle"),
        ("ablation", lambda r: r.get("edge_scorer") == "link_context_sentence_transformer"
         and r.get("profile_name") == "st_future_heavy"
         and r.get("lookahead_depth") == "2", "Single-Path Walk (ST, la2)"),
        ("baseline", lambda r: r.get("baseline") == "dense"
         and r.get("seed_top_k") == "1", "Dense (top-1)"),
        ("baseline", lambda r: r.get("baseline") == "iterative_dense"
         and r.get("seed_top_k") == "3", "Iter. Dense (top-3)"),
        ("baseline", lambda r: r.get("baseline") == "mdr_light"
         and r.get("seed_top_k") == "3", "MDR-Light (top-3)"),
        ("baseline", lambda r: r.get("baseline") == "dense"
         and r.get("seed_top_k") == "3", "Dense (top-3)"),
        ("ablation", lambda r: r.get("selector_family") == "diagnostic"
         and "full_corpus" in r.get("selector", ""), "Full Corpus"),
    ]
    ...
    with open(out_path, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["selector", "support_f1", "support_precision",
                           "support_recall", "budget_utilization"]
        )
        writer.writeheader()
...
def extract_table6():
    ...
    filtered = [
        r for r in rows
        if r.get("selector_family") == "path_search"
        and r.get("budget_value") == budget
    ]
    ...
    with open(out_path, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["edge_scorer", "profile", "lookahead",
                           "support_f1", "support_recall", "support_precision"]
        )
        writer.writeheader()
        writer.writerows(out_rows)
    print(f"Wrote {out_path} ({len(out_rows)} rows)")
