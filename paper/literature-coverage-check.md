...
Date: 2026-03-23
Scope: DBLP + arXiv coverage check for recent (2025-mid 2026) work relevant to hypercorpus.
...
- **Relevance to hypercorpus**: HippoRAG was already the strongest GraphRAG narrative comparator. HippoRAG 2 strengthens their system further. We must cite and discuss this upgrade; it potentially narrows hypercorpus's advantage on the "no eager graph construction" axis if HippoRAG 2 shows lower construction overhead.
...
- **Relevance to hypercorpus**: Represents the "agentic reasoning + retrieval" paradigm that competes with hypercorpus's structured-graph-walk approach. The comparison point is: hypercorpus uses corpus structure (hyperlinks) as navigation signal, while these systems use LLM reasoning chains to decide what to retrieve next from a flat index.
...
- **Relevance to hypercorpus**: MemoRAG's two-stage approach (global memory for clue generation, then targeted retrieval) is structurally analogous to hypercorpus's dense-seed-then-walk approach. Worth citing as a parallel design that operates over flat corpora rather than linked graphs.
...
- **Relevance to hypercorpus**: **This is the most direct potential threat to the novelty claim.** If this paper formulates token-budget-constrained evidence selection as a primary problem (rather than as a downstream context-window optimization), it directly overlaps with hypercorpus's claimed novelty. However, the key distinction is likely that TRACE operates on already-retrieved flat passage sets (post-retrieval compression/selection) while hypercorpus performs budget-aware *graph traversal* to discover evidence in the first place.
...
- **Relevance to hypercorpus**: Strengthens the evidence that budget-aware retrieval is an emerging concern. Webwalker should frame itself within this trend while emphasizing the graph-structure angle.
...
- **Relevance to hypercorpus**: Another system that extracts subgraphs before LLM generation, but uses learned GNN reasoning rather than link-following. Strengthens Section 3's argument that hypercorpus takes a lighter-weight, zero-shot approach compared to learned graph reasoners.
...
- **Relevance to hypercorpus**: Part of the subgraph-retrieval family. Webwalker should distinguish itself from this line by emphasizing natural hyperlinks vs. constructed KGs.
...
- **Relevance to hypercorpus**: Webwalker already constrains its traversal to real hyperlinks. These papers validate the design philosophy but from the LLM-reasoning side. Worth citing to show convergence of the "structure-grounded traversal" idea.
...
- **Relevance to hypercorpus**: These are downstream of hypercorpus's selection stage. The key distinction is that hypercorpus's budget constraint operates at the *discovery/selection* level, not the *context compression* level. This distinction must be made clearly.
...
- **Relevance to hypercorpus**: Represents the opposite architectural choice from hypercorpus's selector-first design. Useful as a "why not end-to-end?" foil in the related work.
...
- **Relevance to hypercorpus**: If any benchmark includes explicit token-budget evaluation, it strengthens hypercorpus's problem formulation. FRAMES (Factuality Retrieval Augmented Evaluation) is particularly relevant as it tests multi-step retrieval.
...
- **Relevance to hypercorpus**: Strengthens the baseline family in Section 2. If any such paper claims strong results with adaptive stopping under budget constraints, it is a direct competitor.
...
- **Assessment**: The existing inventory already lists this as a risk. The key defense is that hypercorpus's budget operates at the *graph traversal/discovery* level (deciding which links to follow and when to stop walking), not at the *post-retrieval compression* level. This distinction must be sharp in the paper.
...
- **Threat**: HippoRAG 2 may reduce the overhead of its knowledge graph construction, weakening hypercorpus's "no eager graph construction" advantage.
- **Assessment**: Even if construction becomes cheaper, the fundamental difference remains: hypercorpus operates on *existing* hyperlinks in the corpus, requiring no construction step at all. HippoRAG 2 still requires indexing-time graph building.
...
- **Threat**: Systems like PRISM, Search-o1, and R1-Searcher frame multi-step retrieval as an LLM reasoning problem, potentially subsuming hypercorpus's structured walk as just one possible action in a more general agentic framework.
...
- **Threat**: Walk&Retrieve (Bockling et al., 2025, IR-RAG@SIGIR) performs zero-shot retrieval via knowledge graph walks. If reviewers see this as the same idea applied to KGs, they may view hypercorpus as incremental.
- **Assessment**: Walk&Retrieve operates over explicit KGs (constructed triples), not natural hyperlinks. hypercorpus's use of anchor text, sentence context, and the existing hyperlink topology is fundamentally different from walking a KG with entity-relation-entity triples.
- **Mitigation**: The differentiation is already noted in the inventory. Strengthen the distinction in the paper text: hypercorpus's graph is not constructed -- it is the corpus itself, with edges carrying rich contextual semantics (anchor text, surrounding sentences) rather than typed relation labels.

### Risk 5: New CIKM 2026 Submissions (UNKNOWN)
- **Threat**: Since hypercorpus targets CIKM 2026, there may be concurrent submissions with overlapping ideas that we cannot yet see.
...
3. **TRACE** (Arefeen et al., 2025) -- Must add and carefully differentiate. TRACE does post-retrieval budget-aware selection; hypercorpus does budget-aware graph discovery.
4. **Agentic retrieval paragraph** -- Add a brief paragraph covering Search-o1/R1-Searcher/ReSearch-style systems to acknowledge the agentic-reasoning-for-retrieval trend. Differentiate on cost: these systems spend budget on reasoning tokens; hypercorpus spends budget on evidence tokens.
...
- **Relevance**: P0 confirmed. The "non-parametric continual learning" framing adds a new angle vs. hypercorpus's "budgeted corpus selection" framing. Must cite and differentiate.
...
- **Relevance**: P1 confirmed. GNN-based subgraph reasoning over KGs, then shortest paths as LLM context. Contrast with hypercorpus's zero-shot, link-following approach.
...
- **Relevance**: P2 confirmed. Memory-inspired clue generation then targeted retrieval -- structurally parallel to hypercorpus's seed-then-walk.
...
- **Relevance**: **P0. This is the actual threat paper for Section 4**, replacing TRACE. It directly formulates token-budgeted context selection as its primary problem. Key distinction from hypercorpus: AdaGReS operates on already-retrieved flat passage sets (post-retrieval selection), while hypercorpus performs budget-aware graph *discovery*. The submodularity framing is worth engaging with.
...
- **Relevance**: P2. Directly compares one-shot-under-budget vs. iterative strategies, which maps onto hypercorpus's budget-constrained walk vs. MDR-style iteration.
...
- **Relevance**: P2. Structure-preserving retrieval under token budget. Analogous to hypercorpus exploiting document link structure, but for hierarchical (tree) rather than graph (hyperlink) structure.
...
- **Relevance**: P2. Modular GraphRAG taxonomy at a top DB venue. Useful for positioning hypercorpus's approach within the GraphRAG design space.
...
- **Relevance**: P2. Graph-structured multi-hop retrieval at a top venue. Different from hypercorpus (lexical graph vs. natural hyperlink graph) but same problem space.
...
- **Relevance**: P2. Agentic graph reasoning via MCTS. High cost (RL training + MCTS inference), strengthens hypercorpus's lightweight-walk argument.
...
These are not new (2022) but may not be in the existing inventory. They validate that hyperlinks as retrieval signals is an established idea, which strengthens hypercorpus's contribution as *budgeted selection* over hyperlinks rather than just *using* hyperlinks.
...
| Agentic retrieval | **Medium** | Search-o1 (EMNLP 2025), R1-Searcher (2503.05592), R1-Searcher++ (2505.17005), ReSearch (2503.19470), Graph-O1 (2512.17912) | Add agentic-retrieval paragraph to Section 4. Note cost contrast with hypercorpus. |
| Multi-hop graph retrieval at top venues | **Medium** | GNN-RAG (ACL 2025), Hierarchical Lexical Graph (KDD 2025), LEGO-GraphRAG (PVLDB 2025) | Ensures hypercorpus is positioned against current top-venue work. |
