...
- Why token-budgeted evidence assembly is the right problem surface.
  - Argument: `gold_support_context` only fits within budget 256 in 63% of 2Wiki 100-case sample (adherence = 0.63); budget is a real constraint.
...
- Contributions paragraph:
  ...
  - Safe components to list now: problem formulation, budget-aware selector family, hyperlink-local evidence assembly, `budget_fill_relative_drop`.
...
**Status: setup description evidence-ready; 2Wiki results ready; full-IIRC results blocked on WS1/ACT-5.**
...
- **Datasets:**
  ...
  - `IIRC` (dev set): main harder-dataset judgment surface.
    - Partial store had 5,184 articles; full-context store has 61,304 articles.
    - **[BLOCKED]**: Full-IIRC canonical store not yet landed. All partial-IIRC results deprecated.
    - 100-case canonical sample planned. Source: `runs/iirc-sample-s100-dense-v1/chunks/chunk-00000/evaluated_case_ids.txt`.
- **Metrics:**
  ...
  - Diagnostic: `selection_runtime_s`, `selector_total_tokens` (cost), `selected_nodes_count`.
  - QA metrics (`answer_em`, `answer_f1`) as reviewer-facing sanity checks only; not primary.
- **Budgets:** `384` and `512` tokens for paper-facing tables. `128` and `256` for calibration.
- **Comparison class:**
...
- **Table 1 (main):** Full-IIRC comparison at budgets `384` and `512`.
  - **[BLOCKED]**: Requires full-IIRC canonical store + all shortlist selectors + real MDR.
  - Data will come from: `runs/iirc-controller-shortlist-v1` (rerun on full store).
...
- **Budget sensitivity.**
  ...
  - Source: `runs/2wiki-baseline-retest-s100-v1`
  - **[BLOCKED]**: Full budget sensitivity analysis requires full-IIRC at 384 and 512.
...
| Full-IIRC canonical store experiments | **Not started** | Table 1 (main), Analysis |
