...
def generate_table1(run_dir: Path, out_dir: Path) -> None:
    ...
    filtered = [
        _add_short_name(r)
        for r in rows
        if r["selector_name"] in TABLE1_SELECTORS and r["budget_label"] in TABLE1_BUDGETS
    ]
...
def generate_table2(run_dir: Path, out_dir: Path) -> None:
    ...
    fields = ["short_name", "subset_label"] + TABLE1_COLUMNS + [
...
def generate_figure_csvs(run_dir: Path, out_dir: Path) -> None:
    ...
    fig3_fields = ["short_name", "selector", "budget_value", "support_f1_zero_on_empty", "support_precision", "support_recall"]
    _write_csv(out_dir / "figure3_budget_sensitivity.csv", fig3, fig3_fields)


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("--run-dir", type=Path, required=True, help="Merged run directory (e.g. runs/iirc-shortlist-full-v1)")
    parser.add_argument("--out-dir", type=Path, default=Path("paper/data"), help="Output directory for paper CSVs")
