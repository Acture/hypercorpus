...
def write_normalized_dataset(
    ...
    *,
    ...
    graph_path.write_text(
        "\n".join(json.dumps(record, ensure_ascii=False) for record in graph_records_list) + ("\n" if graph_records_list else ""),
        encoding="utf-8",
    )
    manifest_payload = {
        "dataset_name": dataset_name,
        "variant": variant,
        "questions": {split: str(path.relative_to(resolved_output)) for split, path in sorted(question_paths.items())},
        "graph": str(graph_path.relative_to(resolved_output)),
        "source_manifest_path": str(Path(source_manifest_path)) if source_manifest_path is not None else None,
