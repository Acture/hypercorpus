    ...
    *,
    ...
    for record in records:
        support_nodes = dedupe_strings(
            _coerce_node_refs(pick_first(record, "gold_support_nodes", "supporting_pages", "supporting_docs"))
        )
...
class DocumentationAdapter(BaseDatasetAdapter):
    ...
    def load_cases(
        ...
        *,
        ...
        return load_docs_questions(questions_source, limit=limit, dataset_name=self.dataset_name)
...
class _DocsHTMLParser(HTMLParser):
    ...
    def handle_endtag(self, tag: str) -> None:
        if tag == "title":
            self._inside_title = False
            return
        if tag == "a" and self._active_block is not None and self._active_anchor is not None:
...
def _build_docs_records_from_html_dir(root: Path, *, dataset_name: str) -> list[dict[str, Any]]:
    html_paths = sorted(
        path
