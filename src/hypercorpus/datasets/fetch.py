...
def _prepare_archive(
    ...
    *,
    ...
    with tempfile.NamedTemporaryFile(prefix="hypercorpus-", suffix=f"-{archive_name}", delete=False) as handle:
...
def _download_url(source_url: str, destination: Path) -> None:
    ...
    with urllib.request.urlopen(request_or_url) as response, destination.open("wb") as handle:
        ...
        if expected_bytes is not None:
            ...
            logger.info("Downloading %s (%.1f MB) to %s", normalized_url, size_mb, destination)
        else:
            logger.info("Downloading %s (unknown size) to %s", normalized_url, destination)
        copy_stream_with_progress(
            response,
            handle,
...
def _download_google_drive(source_url: str, destination: Path) -> None:
    ...
    request = urllib.request.Request(source_url, headers={"User-Agent": "hypercorpus/0.1"})
    with opener.open(request) as response:
        ...
        if "text/html" in content_type:
            ...
            if confirm_match:
                ...
                request = urllib.request.Request(confirmed_url, headers={"User-Agent": "hypercorpus/0.1"})
