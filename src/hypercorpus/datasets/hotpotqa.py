...
def load_hotpotqa_graph(
    ...
    *,
    ...
    if variant == "distractor":
        ...
        return LinkContextGraph.from_normalized_records(normalized, dataset_name="hotpotqa-distractor")
    records = load_json_records(graph_records_path)
    normalized = [_normalize_hotpotqa_graph_record(record, dataset_name="hotpotqa-fullwiki") for record in records]
    return LinkContextGraph.from_normalized_records(normalized, dataset_name="hotpotqa-fullwiki")
...
class HotpotQAAdapter(BaseDatasetAdapter):
    def __init__(self, variant: HotpotVariant = "fullwiki"):
        ...
        self.dataset_name = "hotpotqa-distractor" if variant == "distractor" else "hotpotqa-fullwiki"
...
def build_hotpotqa_distractor_graph_for_case(record: Mapping[str, Any]) -> LinkContextGraph:
...
def _support_nodes_from_record(
    ...
    *,
    ...
    titles = [_normalize_title(title) for title, _sent_idx in supporting_facts if _normalize_title(title)]
    if variant == "distractor":
        return [_case_node_id(case_id, title) for title in titles]
    return titles


def _normalize_hotpotqa_graph_record(record: Mapping[str, Any], *, dataset_name: str) -> dict[str, Any]:
    links = []
