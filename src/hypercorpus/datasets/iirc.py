	...
	*,
	...
	for record in records:
		support_nodes = dedupe_strings(
			_coerce_node_refs(
				pick_first(
					record, "gold_support_nodes", "supporting_docs", "supporting_pages"
				)
			)
		)
		start_nodes = dedupe_strings(
			_coerce_node_refs(
				pick_first(
					record, "gold_start_nodes", "start_nodes", "context_documents"
				)
			)
			or support_nodes
		)
		...
		cases.append(
			EvaluationCase(
				case_id=str(
					pick_first(record, "case_id", "id", "qid", "question_id") or ""
				),
...
def convert_iirc_raw_dataset(
	...
	*,
	...
	for split_name, questions_path in question_files.items():
		...
		question_splits[split_name] = _convert_iirc_questions_payload(
			payload, title_aliases=title_aliases
		)
...
def _normalize_iirc_graph_record(record: Mapping[str, Any]) -> dict[str, Any]:
	...
	for link in pick_first(record, "links", "mentions", "outbound_links") or ():
		...
		if not isinstance(link, Mapping):
			continue
		target = str(
			pick_first(link, "target", "target_id", "title", "page", "ref_url") or ""
		).strip()
		if not target:
			continue
		links.append(
...
def _register_iirc_title_alias(
	title_aliases: dict[str, str], raw_title: Any
) -> str | None:
		...
		for passage_record in payload:
			...
			for question in passage_record.get("questions", []) or []:
				if not isinstance(question, Mapping):
					continue
				for raw_title in _iter_iirc_question_title_refs(
...
def _iter_iirc_question_title_refs(
	question: Mapping[str, Any], *, source_title: str | None
) -> Iterable[str]:
...
def _canonical_iirc_title_refs(
	...
	*,
	...
	for ref in refs:
		canonical = _resolve_iirc_title_alias(
			ref, title_aliases=title_aliases, source_title=source_title
		)
...
def _convert_iirc_questions_payload(
	payload: Any, *, title_aliases: Mapping[str, str]
) -> list[EvaluationCase]:
	if not isinstance(payload, list):
	...
	for passage_record in payload:
		...
		for question in passage_record.get("questions", []) or []:
			...
			question_links = _canonical_iirc_title_refs(
				[
					str(item).strip()
					for item in question.get("question_links", []) or []
			...
			path_nodes = dedupe_strings(
				([source_title] if source_title else []) + question_links
			)
			cases.append(
				EvaluationCase(
					case_id=str(
						pick_first(question, "qid", "question_id", "id", "case_id")
						or ""
					),
	...
	for questions_path in question_files.values():
		...
		for record in payload:
			if not isinstance(record, Mapping):
				continue
			normalized = _normalize_iirc_raw_passage_record(
				record, title_aliases=title_aliases
			)
			graph_records[normalized["node_id"]] = normalized
	if context_source is not None:
		context_payload = json.loads(context_source.read_text(encoding="utf-8"))
		for record in _iter_iirc_context_records(
			context_payload, title_aliases=title_aliases
		):
			normalized = _normalize_iirc_raw_passage_record(
				record, title_aliases=title_aliases
			)
...
def _normalize_iirc_raw_passage_record(
	...
	*,
	...
	sentences = (
		[text] if isinstance(text, str) and text.strip() else _coerce_sentences(record)
	)
	links = []
	for link in record.get("links", []) or []:
		if not isinstance(link, Mapping):
	...
	if isinstance(payload, Mapping):
		for key, value in payload.items():
			if isinstance(value, Mapping):
				...
				merged.setdefault(
					"title",
					_resolve_iirc_title_alias(key, title_aliases=title_aliases)
					or str(key),
				)
				yield merged
...
def _build_iirc_html_context_record(
	*,
	...
	title = _resolve_iirc_title_alias(raw_title, title_aliases=title_aliases) or (
		_clean_iirc_title(raw_title) or raw_title
	)
	sentences: list[str] = []
...
def question_context_passages(
	question: Mapping[str, Any], *, source_title: str | None = None
) -> list[str]:
	passages: list[str] = []
	...
	if isinstance(answer, Mapping):
		...
		if spans:
			texts = [
				...
				for span in spans
				if isinstance(span, Mapping) and str(span.get("text", "")).strip()
			]
			if texts:
				return " ".join(texts)
