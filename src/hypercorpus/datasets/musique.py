		...
		cases.append(
			EvaluationCase(
				...
				expected_answer=_string_or_none(
					pick_first(record, "expected_answer", "answer", "answer_alias")
				),
...
def _convert_musique_question_record(record: Mapping[str, Any]) -> EvaluationCase:
	...
	support_nodes = dedupe_strings(
		_support_node_ids_from_record(
			record,
			node_ids_by_index=node_ids_by_index,
			node_ids_by_title=node_ids_by_title,
		)
	)
	path_nodes = dedupe_strings(
		_path_node_ids_from_record(
			record,
			node_ids_by_index=node_ids_by_index,
			node_ids_by_title=node_ids_by_title,
		)
	...
	start_nodes = (
		path_nodes[:1] or support_nodes[:1] or list(node_ids_by_index.values())[:1]
	)
...
def _convert_musique_graph_records(record: Mapping[str, Any]) -> list[dict[str, Any]]:
	...
	for index, paragraph in enumerate(_raw_musique_paragraphs(record)):
		title = (
			_normalize_title(pick_first(paragraph, "title", "page", "node_id", "id"))
			or f"paragraph-{index}"
		)
		text = str(
			pick_first(paragraph, "paragraph_text", "text", "context", "paragraph")
			or ""
		).strip()
...
def _support_node_ids_from_record(
	...
	*,
	...
	support_indices = pick_first(
		record, "paragraph_support_idx", "supporting_paragraph_indices"
	)
	if isinstance(support_indices, list):
		return [
			node_ids_by_index[index]
			for index in support_indices
			if index in node_ids_by_index
		]
...
def _path_node_ids_from_record(
	...
	*,
	...
	decomposition = pick_first(
		record, "question_decomposition", "reasoning_path", "gold_path_nodes"
	)
	resolved: list[str] = []
	if isinstance(decomposition, list):
		for item in decomposition:
			if isinstance(item, Mapping):
				paragraph_idx = pick_first(
					item, "paragraph_idx", "paragraph_support_idx", "paragraph_index"
				)
				if (
					isinstance(paragraph_idx, int)
					and paragraph_idx in node_ids_by_index
				):
					resolved.append(node_ids_by_index[paragraph_idx])
					continue
				titles = _coerce_node_refs(
					pick_first(item, "title", "page", "node_id", "supporting_title")
				)
				resolved.extend(
					_node_ids_from_titles(titles, node_ids_by_title=node_ids_by_title)
				)
				continue
			if isinstance(item, int) and item in node_ids_by_index:
				...
				continue
			resolved.extend(
				_node_ids_from_titles(
					_coerce_node_refs(item), node_ids_by_title=node_ids_by_title
				)
			)
	return resolved


def _node_ids_from_titles(
	titles: list[str], *, node_ids_by_title: dict[str, list[str]]
) -> list[str]:
	resolved: list[str] = []
	for title in titles:
	...
	for link in pick_first(paragraph, "links", "mentions", "outbound_links") or ():
		if isinstance(link, str):
			...
			if not target:
				continue
			links.append(
				{"target": target, "anchor_text": target, "sentence": "", "sent_idx": 0}
			)
			continue
		if not isinstance(link, Mapping):
			continue
