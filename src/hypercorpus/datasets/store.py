...
class S3CompatibleObjectStore:
	...
	def _client(self) -> Any:
		...
		return boto3.client(
			...
			or os.getenv("WEBWALKER_S3_ENDPOINT_URL"),
	...
	def ensure_local(self, relative_path: str, local_path: Path) -> Path:
		...
		client = self._client()
		logger.info(
			"Downloading dataset store object %s to local cache %s",
			relative_path,
			local_path,
		)
...
class ShardedDocumentStore:
	def __init__(
		...
		*,
		...
		self.catalog_path = self._ensure_metadata_file(
			"index/catalog.sqlite", cache_dir=cache_dir
		)
	...
	def get_document(self, node_id: str) -> DocumentNode | None:
		...
		document = DocumentNode(
			...
			title=str(row["title"]),
			sentences=tuple(
				str(sentence) for sentence in record.get("sentences", ()) or ()
			),
			metadata={
				"url": row["url"],
	...
	def topk_similar(
		...
		if len(scored) < k:
			fallback_candidates = (
				candidates if candidate_set is not None else self._nodes
			)
	...
	def store_questions_locally(self) -> dict[str, Path]:
		return {
			Path(path).stem: self.question_path(Path(path).stem)
			for path in self.manifest.questions_files
		}
	...
	def _ensure_metadata_file(
		self, relative_path: str, *, cache_dir: str | Path | None = None
	) -> Path:
		if isinstance(self.store, LocalDirectoryStore):
		...
		if not local_path.exists():
			logger.info(
				"Materializing metadata file %s into %s", relative_path, local_path
			)
	...
	def _load_nodes(self) -> list[str]:
		rows = self._connection.execute(
			"SELECT node_id FROM documents ORDER BY node_id"
		).fetchall()
...
def prepare_store_from_records(
	*,
	...
	version = _store_version(
		dataset_name, questions_source, graph_source, target_shard_size_bytes
	)

	with sqlite3.connect(catalog_path) as conn:
		_create_catalog_schema(conn)
		writer = _ShardWriter(
			output_root / "shards", target_shard_size_bytes=target_shard_size_bytes
		)
		try:
	...
	manifest = DatasetStoreManifest(
		...
		questions_files=[
			str(Path("questions") / path.name)
			for path in sorted(questions_paths.values())
		],
		shards=shard_infos,
	)
...
def ensure_min_free_space(
	path: Path, *, min_free_gib: float, expected_new_bytes: int | None
) -> None:
	_ensure_min_free_space(
		path, min_free_gib=min_free_gib, expected_new_bytes=expected_new_bytes
	)
...
def _question_loader_for_dataset(dataset_name: str):
	...
	if dataset_name == "hotpotqa-fullwiki":
		return lambda path, limit=None: load_hotpotqa_questions(
			path, limit=limit, variant="fullwiki"
		)
	if dataset_name == "hotpotqa-distractor":
		return lambda path, limit=None: load_hotpotqa_questions(
			path, limit=limit, variant="distractor"
		)
...
def _materialize_source(
	...
	*,
	...
	if keep_raw:
		...
		destination = raw_dir / _basename_from_source(
			source.source, default="source.bin"
		)
		logger.info(
			"Downloading %s source from %s into %s", label, source.source, destination
		)
...
def _write_questions(output_dir: Path, questions_source_path: Path) -> dict[str, Path]:
	...
	def _copy_question_file(
		source_path: Path, *, forced_name: str | None = None
	) -> None:
	...
	if suffixes and suffixes[-1] in {".json", ".jsonl"}:
		forced_name = (
			None
			if questions_source_path.stem in STANDARD_SPLITS
			else f"dev{questions_source_path.suffix}"
		)
	...
	if suffixes and suffixes[-1] == ".zip":
		with zipfile.ZipFile(questions_source_path) as archive:
			...
			if not members:
				raise ValueError(
					f"Archive {questions_source_path} does not contain question JSON files"
				)
			single_nonstandard = (
				len(members) == 1 and Path(members[0]).stem not in STANDARD_SPLITS
			)
...
def _iter_normalized_records(path: Path) -> Iterator[dict[str, Any]]:
	if path.is_dir():
		candidates = sorted(
			...
			and (
				...
				or tuple(suffix.lower() for suffix in item.suffixes[-2:])
				in {(".jsonl", ".gz"), (".json", ".gz")}
			)
		)
		if not candidates:
	...
	if suffixes and suffixes[-1] == ".zip":
		with zipfile.ZipFile(path) as archive:
			...
			if len(members) != 1:
				raise ValueError(
					f"Archive {path} must contain exactly one graph JSON/JSONL file"
				)
			member = members[0]
	...
	if isinstance(payload, dict):
		for key in ("records", "documents", "pages", "items"):
			...
			if isinstance(value, list):
				...
				return
	raise ValueError(
		"Unsupported graph payload: expected a list or a dict containing records/documents/pages/items"
	)
...
def _record_url(record: dict[str, Any]) -> str:
	...
	return str(
		record.get("url") or metadata.get("url") or metadata.get("source_path") or ""
	)
	...
	if isinstance(links, list):
		for link in links:
			if not isinstance(link, dict):
				continue
			target = str(
				link.get("target") or link.get("target_id") or link.get("title") or ""
			).strip()
			if not target:
				continue
...
def _download_to_temp(source_url: str) -> Path:
	...
	with tempfile.NamedTemporaryFile(
		prefix="hypercorpus-", suffix=suffix, delete=False
	) as handle:
...
def _store_version(
	...
	digest = hashlib.sha1(
		f"{dataset_name}|{questions_source}|{graph_source}|{target_shard_size_bytes}".encode(
			"utf-8"
		)
	).hexdigest()[:12]
