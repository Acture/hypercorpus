...
def load_2wiki_graph(graph_records_path: str | Path) -> LinkContextGraph:
	...
	logger.info(
		"Building LinkContextGraph from %s normalized 2Wiki records",
	...
	logger.info(
		"Loaded 2Wiki graph from %s with %s nodes", records_path, len(graph.nodes)
	)
	return graph
		...
		with zipfile.ZipFile(resolved) as archive:
			...
			if member_name is None:
				raise FileNotFoundError(
					f"Archive {resolved} does not contain para_with_hyperlink.jsonl"
				)
...
def default_2wiki_raw_paths(
	root: str | Path = TWOWIKI_DEFAULT_ROOT,
) -> tuple[Path, Path]:
	resolved = Path(root)
	return (
		resolved / "data_ids_april7" / "dev.json",
		resolved / "para_with_hyperlink.jsonl",
	)
...
def _iter_records_with_progress_bytes(
	path: Path,
	...
	if suffixes[-2:] == [".jsonl", ".gz"]:
		with path.open("rb") as raw_handle:
			with gzip.GzipFile(fileobj=raw_handle, mode="rb") as gzip_handle:
				...
				yield from _iter_jsonl_lines_with_progress(
					text_handle, lambda: raw_handle.tell()
				)
		return
	if suffixes and suffixes[-1] == ".jsonl":
		with path.open("rb") as raw_handle:
	...
	if suffixes and suffixes[-1] == ".zip":
		with zipfile.ZipFile(path) as archive:
			...
			if member_name is None:
				raise FileNotFoundError(
					f"Archive {path} does not contain para_with_hyperlink.jsonl"
				)
			with archive.open(member_name, "r") as handle:
