...
class EvaluationCase:
	...
	def __post_init__(self) -> None:
		...
		self.gold_start_nodes = _dedupe(
			self.gold_start_nodes or self.gold_support_nodes
		)
...
class EvaluationBudget:
	...
	def __post_init__(self) -> None:
		...
		if (self.token_budget_tokens is None) == (self.token_budget_ratio is None):
			raise ValueError(
				"EvaluationBudget requires exactly one of token_budget_tokens or token_budget_ratio."
			)
		if self.token_budget_tokens is not None:
			if self.token_budget_tokens <= 0:
				raise ValueError(
					"EvaluationBudget.token_budget_tokens must be positive."
				)
...
class _SelectorBudgetAccumulator:
	...
	avg_support_precision: _AverageAccumulator = field(
	...
	avg_selected_token_estimate: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_compression_ratio: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_budget_adherence: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_budget_utilization: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_empty_selection_rate: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_selection_runtime_s: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_support_f1_zero_on_empty: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_support_set_em: _AverageAccumulator = field(default_factory=_AverageAccumulator)
	avg_selector_prompt_tokens: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_selector_completion_tokens: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_selector_total_tokens: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_selector_runtime_s: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_selector_llm_calls: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	avg_selector_fallback_rate: _AverageAccumulator = field(
		default_factory=_AverageAccumulator
	)
	...
	def add_result(self, result: SelectionResult) -> None:
		...
		if usage is not None:
			...
			if usage.step_count > 0:
				self.avg_selector_fallback_rate.add(
					usage.fallback_steps / usage.step_count
				)
...
class IncrementalExperimentAggregator:
	def __init__(self, *, dataset_name: str):
		...
		self.total_cases = 0
		self._buckets: dict[
			tuple[str, str, int | float, str | None, str | None],
			_SelectorBudgetAccumulator,
		] = {}
	...
	def add_selection_result(self, selection: SelectionResult) -> None:
		key = (
			...
			selection.selector_metadata.provider
			if selection.selector_metadata is not None
			else None,
			selection.selector_metadata.model
			if selection.selector_metadata is not None
			else None,
		)
...
class Evaluator:
	...
	def evaluate_case(
		self, graph: LinkContextGraph, case: EvaluationCase
	) -> CaseEvaluation:
		selections: list[SelectionResult] = []
	...
	def summarize(self, evaluations: Sequence[CaseEvaluation]) -> ExperimentSummary:
		...
		return summarize_evaluations(
			evaluations, dataset_name=evaluations[0].case.dataset_name
		)
...
def _selection_result_from_raw(
	*,
	...
	metrics = SelectionMetrics(
		...
		selection_runtime_s=raw.selector_usage.runtime_s
		if raw.selector_usage is not None
		else 0.0,
		...
		budget_utilization=_budget_utilization(
			corpus.token_estimate, budget_token_limit
		),
		...
		support_f1_zero_on_empty=_support_f1_zero_on_empty(
			corpus.node_ids, case.gold_support_nodes
		),
...
def _start_hit(
	root_node_ids: Sequence[str], gold_start_nodes: Sequence[str]
) -> bool | None:
...
def _precision(
	selected_nodes: Sequence[str], gold_nodes: Sequence[str]
) -> float | None:
...
def _path_hit(
	selected_nodes: Sequence[str], gold_path_nodes: Sequence[str] | None
) -> bool | None:
