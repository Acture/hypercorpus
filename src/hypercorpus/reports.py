...
def load_result_records(results_path: str | Path | Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
...
def subset_comparison_rows(
    ...
    *,
    ...
    for subset_label, predicate in _subset_predicates():
        ...
        control_by_budget = {
            budget_label: _aggregate_subset_rows(grouped.get((control_selector_name, budget_label), []))
        ...
        for selector_name, budget_label in selector_budget_keys:
            ...
            aggregate = _aggregate_subset_rows(grouped.get((selector_name, budget_label), []))
            ...
            rows.append(
                {
                    ...
                    "delta_support_f1_vs_dense_control": _subset_delta(
                        ...
                        None if control is None else control["avg_support_f1_zero_on_empty"],
                    ),
                    ...
                    "avg_prefiltered_candidates": aggregate["avg_prefiltered_candidates"],
                }
            )
    return rows
...
def _subset_predicates() -> list[tuple[str, Any]]:
    return [
        ...
        (
            ...
            lambda record: set(record.get("gold_start_nodes", [])) != set(record.get("gold_support_nodes", [])),
        ),
    ]
...
def _aggregate_subset_rows(rows: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    for record in rows:
        ...
        if calls > 0:
            ...
            backtrack_rates.append(float(usage.get("controller_backtrack_actions", 0)) / calls)
            prefiltered_candidates.append(float(usage.get("controller_prefiltered_candidates", 0)) / calls)
        else:
