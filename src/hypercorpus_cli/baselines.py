...
def export_mdr_iirc(
	...
	output_dir: Path = typer.Option(
		..., "--output-dir", file_okay=False, help="Directory for exported MDR files"
	),
...
def train_mdr(
	...
	export_manifest: Path = typer.Option(
		...
		exists=True,
		dir_okay=False,
		help="Export manifest produced by export-mdr-iirc",
	),
	output_dir: Path = typer.Option(
		...,
		"--output-dir",
		file_okay=False,
		help="Wrapper output directory for official MDR training",
	),
	init_checkpoint: Path | None = typer.Option(
		...
		exists=True,
		dir_okay=False,
		help="Public checkpoint used to initialize MDR",
	),
	...
	model_name: str = typer.Option(
		"roberta-base", "--model-name", help="Official MDR model name"
	),
	shared_encoder: bool = typer.Option(
		False,
		"--shared-encoder/--no-shared-encoder",
		help="Forward the shared-encoder flag to official MDR",
	),
) -> None:
...
def build_index(
	...
	output_dir: Path = typer.Option(
		...
		"--output-dir",
		file_okay=False,
		help="Directory for encoded corpus artifacts",
	),
	checkpoint: Path | None = typer.Option(
		...
		exists=True,
		dir_okay=False,
		help="Trained or public MDR checkpoint",
	),
	train_manifest: Path | None = typer.Option(
		...
		exists=True,
		dir_okay=False,
		help="Optional training manifest from train-mdr",
	),
	mdr_home: Path | None = typer.Option(
		None,
		"--mdr-home",
	...
	model_name: str | None = typer.Option(
		None, "--model-name", help="Override the model name used for corpus encoding"
	),
	shared_encoder: bool | None = typer.Option(
		None,
		"--shared-encoder/--no-shared-encoder",
		help="Override the shared-encoder flag used for corpus encoding",
	),
	max_q_len: int = typer.Option(
		70, "--max-q-len", min=1, help="Question encoder max length"
	),
	max_c_len: int = typer.Option(
		300, "--max-c-len", min=1, help="Corpus encoder max length"
	),
	max_q_sp_len: int = typer.Option(
		350, "--max-q-sp-len", min=1, help="Second-hop query max length"
	),
	beam_size: int = typer.Option(
		5,
		"--beam-size",
		min=1,
		help="Default first-hop beam size recorded in the artifact manifest",
	),
	topk_paths: int = typer.Option(
		10,
		"--topk-paths",
		min=1,
		help="Default number of 2-hop paths recorded in the artifact manifest",
	),
) -> None:
