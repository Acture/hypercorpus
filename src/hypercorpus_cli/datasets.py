...
def fetch_2wiki(
	output_dir: Path = typer.Option(
		..., "--output-dir", file_okay=False, help="Directory for extracted files"
	),
	split: str = typer.Option(
		"dev",
		"--split",
		help=f"Question split to fetch: {', '.join([*TWOWIKI_SPLITS, 'all'])}",
	),
	questions: bool = typer.Option(
		True, "--questions/--no-questions", help="Fetch question JSON files"
	),
	...
	overwrite: bool = typer.Option(
		False, "--overwrite", help="Overwrite any existing extracted files"
	),
	min_free_gib: float = typer.Option(
		DEFAULT_MIN_FREE_GIB,
		"--min-free-gib",
		min=1.0,
		help="Minimum free space to preserve",
	),
	yes: bool = typer.Option(
		False,
		"--yes",
		help="Skip interactive confirmation before downloading the large graph",
	),
) -> None:
	console = Console()
	estimated_bytes = 0
	...
	ensure_min_free_space(
		...
		expected_new_bytes=estimated_bytes or None,
	)
	if graph and not yes:
		graph_size = probe_source_size(TWOWIKI_GRAPH_URL)
		size_text = (
			_format_size(graph_size) if graph_size is not None else "unknown size"
		)
		if not typer.confirm(
			f"Download the shared 2Wiki graph archive ({size_text}) into {output_dir}?"
		):
			raise typer.Abort()
	...
	if layout.graph_path is None:
		console.print(
			"graph not downloaded: add --graph when you are ready for the large shared file."
		)
	console.print(
		"for a zero-download smoke dataset: hypercorpus-cli datasets write-2wiki-sample --output-dir /tmp/hypercorpus-2wiki-sample"
	)
...
def write_2wiki_sample(
	output_dir: Path = typer.Option(
		..., "--output-dir", file_okay=False, help="Directory for the sample dataset"
	),
	overwrite: bool = typer.Option(
		False, "--overwrite", help="Overwrite any existing sample files"
	),
) -> None:
	console = Console()
...
def fetch_iirc(
	output_dir: Path = typer.Option(
		...
		"--output-dir",
		file_okay=False,
		help="Directory for downloaded raw artifacts",
	),
	archive_url: str = typer.Option(
		IIRC_ARCHIVE_URL,
		"--archive-url",
		help="Override URL for the IIRC train/dev archive",
	),
	context_source: str | None = typer.Option(
		None,
		"--context-source",
		help="Override the built-in IIRC context source with a local path or URL to context_articles.json or context_articles.tar.gz",
	),
	question_only: bool = typer.Option(
		False,
		"--question-only",
		help="Skip downloading the built-in IIRC context corpus and allow a partial fetch",
	),
	overwrite: bool = typer.Option(
		False, "--overwrite", help="Overwrite any existing downloaded/extracted files"
	),
) -> None:
	console = Console()
	try:
	...
	if question_only:
		console.print(
			"warning: fetched IIRC raw artifacts do not guarantee a complete context graph; convert/store results may be partial."
		)
	console.print("next:")
	console.print(
		f"uv run hypercorpus-cli datasets convert-iirc-raw --raw-dir {layout.raw_dir / 'iirc'} --output-dir {output_dir / 'normalized'}"
	)
...
def fetch_musique(
	...
	split: str = typer.Option(
		"dev", "--split", help="Question split to fetch: train, dev, test, or all"
	),
	subset: str = typer.Option("full", "--subset", help="MuSiQue subset: full or ans"),
	train_url: str | None = typer.Option(
		None, "--train-url", help="Override the train questions URL"
	),
	dev_url: str | None = typer.Option(
		None, "--dev-url", help="Override the dev questions URL"
	),
	test_url: str | None = typer.Option(
		None, "--test-url", help="Override the test questions URL"
	),
	overwrite: bool = typer.Option(
		False, "--overwrite", help="Overwrite any existing downloaded files"
	),
) -> None:
	console = Console()
	split_urls = {
	...
	default_urls = (
		MUSIQUE_FULL_SPLIT_URLS if subset == "full" else MUSIQUE_ANSWERABLE_SPLIT_URLS
	)
	resolved_urls = {
		name: override or default_urls[name] for name, override in split_urls.items()
	}
	...
	console.print("next:")
	console.print(
		f"uv run hypercorpus-cli datasets convert-musique-raw --raw-dir {layout.raw_dir / 'musique' / subset} --output-dir {output_dir / 'normalized'}"
	)
...
def fetch_hotpotqa(
	...
	variant: str = typer.Option(
		"distractor", "--variant", help="HotpotQA variant: distractor or fullwiki"
	),
	split: str = typer.Option(
		"dev", "--split", help="Question split to fetch: train, dev, or all"
	),
	train_url: str | None = typer.Option(
		None, "--train-url", help="Override the train questions URL"
	),
	...
	defaults = (
		...
		else HOTPOTQA_FULLWIKI_SPLIT_URLS
	)
	resolved_urls = {
	...
	layout = fetch_hotpotqa_dataset(
		...
		split=split,
		split_urls={
			name: url for name, url in resolved_urls.items() if url is not None
		},
		overwrite=overwrite,
	)
	_print_raw_layout(console, layout)
...
def convert_iirc_raw(
	raw_dir: Path = typer.Option(
		...
		file_okay=True,
		dir_okay=True,
		help="Path to fetched IIRC raw artifacts",
	),
	output_dir: Path = typer.Option(
		...,
		"--output-dir",
		file_okay=False,
		help="Directory for normalized graph/questions",
	),
	overwrite: bool = typer.Option(
		False, "--overwrite", help="Overwrite any existing normalized outputs"
	),
) -> None:
	console = Console()
...
def convert_musique_raw(
	raw_dir: Path = typer.Option(
		...
		file_okay=True,
		dir_okay=True,
		help="Path to fetched MuSiQue raw artifacts",
	),
	output_dir: Path = typer.Option(
		...,
		"--output-dir",
...
def convert_hotpotqa_raw(
	raw_dir: Path = typer.Option(
		...
		file_okay=True,
		dir_okay=True,
		help="Path to fetched HotpotQA raw artifacts",
	),
	output_dir: Path = typer.Option(
		...,
		"--output-dir",
	...
	graph_source: str | None = typer.Option(
		None,
		"--graph-source",
		help="Required for fullwiki conversion; path or URL to a normalized graph bundle",
	),
	overwrite: bool = typer.Option(
		False, "--overwrite", help="Overwrite any existing normalized outputs"
	),
...
def prepare_iirc_store_from_raw(
	output_dir: Path = typer.Option(
		...
		"--output-dir",
		file_okay=False,
		help="Directory containing raw/ normalized/ store/",
	),
	archive_url: str = typer.Option(
		IIRC_ARCHIVE_URL,
		"--archive-url",
	...
	overwrite: bool = typer.Option(
		False, "--overwrite", help="Overwrite any existing outputs under output-dir"
	),
	min_free_gib: float = typer.Option(
		DEFAULT_MIN_FREE_GIB,
		"--min-free-gib",
...
def prepare_hotpotqa_store_from_raw(
	...
	if variant == "fullwiki" and graph_source is None:
		raise typer.BadParameter(
			"HotpotQA fullwiki store preparation requires --graph-source."
		)
	raw_root = output_dir / "raw-fetch"
	normalized_root = output_dir / "normalized"
	store_root = output_dir / "store"
...
def prepare_2wiki_store_cli(
	output_dir: Path = typer.Option(
		...
		"--output-dir",
		file_okay=False,
		help="Directory for the prepared sharded store",
	),
	questions_source: str | None = typer.Option(
		None,
		"--questions-source",
		help="Local path or URL to 2Wiki questions json/zip/dir",
	),
	graph_source: str | None = typer.Option(
		None, "--graph-source", help="Local path or URL to 2Wiki graph jsonl/zip"
	),
	keep_raw: bool = typer.Option(
		False,
		"--keep-raw/--no-keep-raw",
		help="Keep downloaded raw graph artifacts under the store root",
	),
	overwrite: bool = typer.Option(
		False, "--overwrite", help="Overwrite any existing prepared store"
	),
	min_free_gib: float = typer.Option(
		DEFAULT_MIN_FREE_GIB,
		"--min-free-gib",
	...
	ensure_min_free_space(
		output_dir,
		min_free_gib=min_free_gib,
		expected_new_bytes=estimate_prepare_bytes(graph_size),
	)
	try:
...
def _print_normalized_layout(console: Console, layout) -> None:
	...
	if layout.graph_path is not None:
		console.print(
			f"graph -> {layout.graph_path} ({_format_path_size(layout.graph_path)})"
		)
...
def _source_manifest_for(raw_dir: Path) -> Path | None:
	...
	for candidate in [
		...
		parent.parent / "source-manifest.json",
		parent.parent.parent / "source-manifest.json",
	]:
		if candidate.exists():
			return candidate
	return None
	...
	*,
	...
	for source in (questions_source, graph_source):
		...
		if (
			...
			or source_dataset == dataset_name
		):
			continue
...
def prepare_iirc_store_cli(
	...
	questions_source: str = typer.Option(
		..., "--questions-source", help="Local path or URL to IIRC questions JSON/JSONL"
	),
	graph_source: str = typer.Option(
		...,
		"--graph-source",
		help="Local path or URL to normalized IIRC graph JSON/JSONL",
	),
	keep_raw: bool = typer.Option(
		False,
		"--keep-raw/--no-keep-raw",
...
def prepare_musique_store_cli(
	...
	questions_source: str = typer.Option(
		...,
		"--questions-source",
		help="Local path or URL to MuSiQue questions JSON/JSONL",
	),
	graph_source: str = typer.Option(
		...,
		"--graph-source",
		help="Local path or URL to normalized MuSiQue graph JSON/JSONL",
	),
	keep_raw: bool = typer.Option(
		False,
		"--keep-raw/--no-keep-raw",
...
def prepare_hotpotqa_store_cli(
	...
	questions_source: str = typer.Option(
		...,
		"--questions-source",
		help="Local path or URL to HotpotQA fullwiki questions JSON/JSONL",
	),
	graph_source: str = typer.Option(
		...,
		"--graph-source",
		help="Local path or URL to normalized HotpotQA fullwiki graph JSON/JSONL",
	),
	variant: str = typer.Option(
		"fullwiki",
		"--variant",
		help="HotpotQA store variant; only fullwiki is supported for stores",
	),
	keep_raw: bool = typer.Option(
		False,
		"--keep-raw/--no-keep-raw",
	...
	if variant != "fullwiki":
		raise typer.BadParameter(
			"HotpotQA distractor runs are direct-only; use --variant fullwiki for prepared stores."
		)
	console = Console()
	_validate_normalized_inputs_match_dataset(
...
def inspect_2wiki_store_cli(
	store: str | None = typer.Option(
		None, "--store", help="Optional prepared store path or s3:// URI"
	),
	cache_dir: Path = typer.Option(
		DEFAULT_CACHE_DIR,
		"--cache-dir",
		file_okay=False,
		help="Local cache directory for prepared stores",
	),
	raw_root: Path | None = typer.Option(
		None, "--raw-root", file_okay=False, help="Optional raw 2Wiki root to inspect"
	),
) -> None:
	console = Console()
	inspection = inspect_2wiki_store(
		store_uri=store, cache_dir=cache_dir, raw_root=raw_root
	)
	console.print(
		f"cache_dir -> {inspection.cache_dir} ({_format_size(inspection.cache_size_bytes)})"
	)
	...
	if inspection.raw_graph_path is not None:
		console.print(
			f"raw_graph -> {inspection.raw_graph_path} ({_format_size(inspection.raw_graph_size_bytes)})"
		)
	...
	if inspection.remote_questions_size_bytes is not None:
		console.print(
			f"remote_questions_zip -> {_format_size(inspection.remote_questions_size_bytes)}"
		)
	if inspection.remote_graph_size_bytes is not None:
		console.print(
			f"remote_graph_zip -> {_format_size(inspection.remote_graph_size_bytes)}"
		)
...
def inspect_store_cli(
	store: str = typer.Argument(
		..., help="Path to a prepared store directory (must contain manifest.json)"
	),
	cache_dir: Path = typer.Option(
		DEFAULT_CACHE_DIR,
		"--cache-dir",
