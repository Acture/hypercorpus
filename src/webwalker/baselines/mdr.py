...
def export_iirc_store_to_mdr(
    *,
    ...
    if store.manifest.dataset_name != "iirc":
        raise ValueError(f"export-mdr-iirc requires an IIRC store, got {store.manifest.dataset_name!r}")
...
def build_mdr_index(
    *,
    ...
    export_manifest = load_mdr_export_manifest(export_manifest_path)
    train_manifest = load_mdr_train_manifest(train_manifest_path) if train_manifest_path is not None else None
    resolved_home, commit_sha, patch_sha = _validate_and_patch_mdr_checkout(mdr_home or (train_manifest.mdr_home if train_manifest else None))
    ...
    resolved_model_name = model_name or (train_manifest.model_name if train_manifest is not None else "roberta-base")
    resolved_shared_encoder = shared_encoder if shared_encoder is not None else (
        train_manifest.shared_encoder if train_manifest is not None else False
    )
    ...
    if not embeddings_path.exists():
        raise FileNotFoundError(f"Official MDR encoding did not produce embeddings at {embeddings_path}")
    id2doc_path = embed_prefix / "id2doc.json"
    if not id2doc_path.exists():
        raise FileNotFoundError(f"Official MDR encoding did not produce id2doc.json at {id2doc_path}")

    embeddings = np.load(embeddings_path).astype("float32")
    index_path = _maybe_build_faiss_index(output_root=output_root, embeddings=embeddings)
    ...
    manifest = MDRArtifactManifest(
        ...
        mdr_patch_sha=patch_sha,
        export_manifest_path=str(Path(export_manifest_path).resolve()),
        train_manifest_path=None if train_manifest_path is None else str(Path(train_manifest_path).resolve()),
...
class ExternalMDRSelector:
    def __init__(
        ...
        *,
        ...
        backend_factory: Callable[[MDRArtifactManifest], _MDRRetrievalBackend] | None = None,
    ) -> None:
        ...
        from webwalker.selector import SelectorSpec

        self.name = name
        self._artifact_manifest_path = None if artifact_manifest_path is None else Path(artifact_manifest_path).resolve()
    ...
    def _get_artifact_manifest(self) -> MDRArtifactManifest:
        if self._artifact_manifest is None:
            ...
            self._artifact_manifest = load_mdr_artifact_manifest(self._artifact_manifest_path)
...
class _OfficialMDRBackend:
    def __init__(
        ...
        *,
        ...
        self.embeddings = np.load(self.manifest.corpus_embeddings_path).astype("float32")
    ...
    def retrieve(
        ...
        *,
        ...
        first_scores, first_indices = self._search(q_embed.detach().cpu().numpy().astype("float32"), topk)
        ...
        second_scores, second_indices = self._search(q_sp_embed.detach().cpu().numpy().astype("float32"), topk)
        ...
        for hop1_rank, hop2_rank in ranked_pairs[: max(1, min(topk_paths, path_scores.size))]:
    ...
    def _search(self, query_vectors: np.ndarray, topk: int) -> tuple[np.ndarray, np.ndarray]:
...
def _validate_and_patch_mdr_checkout(mdr_home: str | Path | None) -> tuple[Path, str, str]:
    ...
    for relative_path in _REQUIRED_MDR_PATHS:
        ...
        if not candidate.exists():
            raise ValueError(f"Official MDR checkout is missing required path: {candidate}")
    ...
    patched, replacements = re.subn(r"(?m)^[ \t]*import pdb; pdb\.set_trace\(\)\n?", "", source)
    if replacements:
...
def _discover_checkpoint(output_root: Path) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(f"No checkpoint_best.pt or checkpoint_last.pt found under {output_root}")
    candidates.sort(key=lambda path: path.stat().st_mtime, reverse=True)
    return candidates[0]


def _maybe_build_faiss_index(*, output_root: Path, embeddings: np.ndarray) -> Path | None:
    try:
...
def _select_start_doc(case: Any, node_set: set[str]) -> str | None:
    for node_id in list(getattr(case, "gold_start_nodes", ()) or ()) + list(getattr(case, "gold_support_nodes", ()) or ()):
