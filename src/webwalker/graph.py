...
from webwalker.candidate.protocol import EmbeddedGraphLike
...
class LinkContextGraph(EmbeddedGraphLike[str]):
    ...
    def __init__(
        ...
        self._links_by_edge: dict[tuple[str, str], list[LinkContext]] = defaultdict(list)
    ...
    def add_link(self, link: LinkContext) -> None:
        ...
        self.ensure_document(link.target, title=link.target.replace("_", " "), placeholder=True)
    ...
    def from_2wikimultihop_records(
        ...
        *,
        ...
        for record in record_list:
            node_id = str(record.get(id_field) or record.get(title_field))
            title = str(record.get(title_field) or node_id).replace("_", " ")
            sentences = tuple(str(sentence) for sentence in record.get(sentences_field, ()) or ())
            ...
            for mention in record.get(mentions_field, ()) or ():
                ...
                if target is None and ref_url:
                    target = title_to_node.get(ref_url) or title_to_node.get(ref_url.replace("_", " "))
                if target is None:
        ...
        return cls.from_normalized_records(normalized_records, dataset_name="2wikimultihop")
...
def _coerce_document_record(
    ...
    *,
    ...
    if sentences_raw:
        sentences = tuple(str(sentence).strip() for sentence in sentences_raw if str(sentence).strip())
    else:
...
def _coerce_link_record(record: NormalizedLinkRecord | Mapping[str, Any]) -> NormalizedLinkRecord:
    if isinstance(record, NormalizedLinkRecord):
        return record
    return NormalizedLinkRecord(
        target=str(record.get("target") or record.get("target_id") or record.get("ref_url") or ""),
...
def _split_sentences(text: str) -> tuple[str, ...]:
    ...
    if not normalized:
        return ()
    return tuple(part.strip() for part in _SENTENCE_SPLIT_RE.split(normalized) if part.strip())
