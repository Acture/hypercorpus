...
def test_run_iirc_store_experiment_matches_direct_dataset_name(
	iirc_files, prepared_iirc_store, tmp_path
):
...
def test_run_hotpotqa_distractor_direct(hotpotqa_distractor_file, tmp_path):
	...
	record = json.loads(
		(tmp_path / "hotpot-distractor" / "results.jsonl")
...
def test_run_hotpotqa_fullwiki_direct_and_store(
	hotpotqa_fullwiki_files, prepared_hotpotqa_store, tmp_path
):
...
def test_merge_store_results_for_new_datasets(
	...
	musique_summary, musique_missing = merge_musique_results(
		run_dir=tmp_path / "runs" / "musique"
	)
	hotpot_summary, hotpot_missing = merge_hotpotqa_results(
		run_dir=tmp_path / "runs" / "hotpot"
	)
