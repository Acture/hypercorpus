...
def _write_iirc_archive(tmp_path: Path, *, include_context: bool, include_appledouble: bool = False) -> Path:
    ...
    archive_path = tmp_path / ("iirc-with-context.tgz" if include_context else "iirc-questions-only.tgz")
    payloads = {
...
def test_fetch_iirc_dataset_can_materialize_context_from_local_path(tmp_path):
    ...
    context_destination = layout.raw_dir / "iirc" / "extracted" / "context_articles.json"
...
def test_fetch_iirc_dataset_can_materialize_context_from_local_archive(tmp_path):
    ...
    manifest = json.loads(layout.source_manifest_path.read_text(encoding="utf-8"))
    by_name = {artifact["name"]: artifact for artifact in manifest["artifacts"]}
    assert by_name["context_archive"]["source_url"] == context_archive.resolve().as_uri()
...
def test_fetch_iirc_dataset_prefers_archive_embedded_context_over_built_in_source(iirc_raw_archive, tmp_path, monkeypatch):
...
def test_fetch_iirc_dataset_allows_question_only_when_requested(tmp_path):
    ...
    assert (layout.raw_dir / "iirc" / "extracted" / "dev.json").exists()
    assert not (layout.raw_dir / "iirc" / "extracted" / "context_articles.json").exists()


def test_fetch_iirc_dataset_ignores_appledouble_json_members(tmp_path):
    archive_path = _write_iirc_archive(tmp_path, include_context=True, include_appledouble=True)
