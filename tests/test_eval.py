...
def test_available_selector_names_are_canonical_only():
	...
	assert (
		"top_3_seed__lexical_overlap__hop_3__beam__link_context_llm__lookahead_2"
		in names
	)
...
def test_selector_names_for_paper_recommended_preset_include_expected_profiles_and_diagnostics():
	...
	assert (
		selector_names_for_preset("paper_recommended", include_diagnostics=False)
		== names[:-2]
	)
...
def test_selector_names_for_paper_recommended_local_preset_exclude_llm_and_keep_diagnostics():
	...
	assert (
		selector_names_for_preset("paper_recommended_local", include_diagnostics=False)
		== names[:-2]
	)
...
def test_selector_names_for_branchy_profiles_preset_are_fixed_and_parseable():
	...
	assert (
		selector_names_for_preset("branchy_profiles", include_diagnostics=False)
		== names[:-2]
	)
	for selector_name in names[:-2]:
		assert parse_selector_spec(selector_name).canonical_name == selector_name
...
def test_parse_selector_spec_rejects_legacy_names():
	...
	with pytest.raises(
		ValueError, match="Unknown selector: adaptive_link_context_walk"
	):
...
def test_evaluator_runs_canonical_selectors(sample_graph):
	...
	results = {
		selection.selector_name: selection for selection in evaluation.selections
	}
	...
	assert (
		results[
			"top_1_seed__lexical_overlap__hop_0__dense"
		].metrics.selected_nodes_count
		== 1
	)
	assert (
		results["top_1_seed__lexical_overlap__hop_0__dense"].metrics.support_set_em
		== 0.0
	)
	assert results["full_corpus_upper_bound"].metrics.compression_ratio == 1.0
	assert (
		results["top_1_seed__lexical_overlap__hop_1__topology_neighbors"].end_to_end
		is not None
	)
	assert (
		results["top_1_seed__lexical_overlap__hop_1__topology_neighbors"].end_to_end.em
		== 1.0
	)
...
def test_incremental_experiment_aggregator_matches_batch_summary():
	...
	assert [
		(row.name, row.selector_provider, row.selector_model)
		for row in incremental_summary.selector_budgets
	] == [
...
def test_empty_selection_metrics_are_visible_without_changing_raw_f1():
	...
	nonempty_result = SelectionResult(
		...
		budget=budget,
		corpus=SelectedCorpus(
			node_ids=["mission"], edge_contexts=[], token_estimate=64
		),
		metrics=SelectionMetrics(
			budget_mode=budget.budget_mode,
			budget_value=budget.budget_value,
