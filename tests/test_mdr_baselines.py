...
def _prepare_iirc_train_dev_store(tmp_path: Path) -> Path:
    ...
    train_path.write_text(json.dumps(train_questions, ensure_ascii=False), encoding="utf-8")
...
def test_export_iirc_store_to_mdr_writes_expected_schema_and_is_stable(tmp_path):
    ...
    train_lines_b = (export_dir_b / "train.jsonl").read_text(encoding="utf-8").splitlines()
...
def test_external_mdr_selector_flattens_paths_and_applies_budget():
    graph = LinkContextGraph(
        ...
        links=[
            LinkContext(source="doc-a", target="doc-b", anchor_text="b", sentence="go to b", sent_idx=0),
            LinkContext(source="doc-b", target="doc-c", anchor_text="c", sentence="go to c", sent_idx=0),
        ],
    )
...
def test_run_iirc_store_cli_requires_mdr_artifact_manifest_for_external_selector(prepared_iirc_store, tmp_path):
    runner = CliRunner()
