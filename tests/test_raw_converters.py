...
def _write_iirc_html_raw_dir(tmp_path: Path) -> Path:
			...
			"questions": [
				{
					...
					"context": [
						...
						{
							"passage": "cape canaveral",
							"text": "Cape Canaveral is in Florida.",
						},
						{
							"passage": "Florida",
							"text": "Florida is a state in the United States.",
						},
					],
				}
			],
	...
	(raw_dir / "train.json").write_text(
		json.dumps(payload, ensure_ascii=False), encoding="utf-8"
	)
	(raw_dir / "dev.json").write_text(
		json.dumps(payload, ensure_ascii=False), encoding="utf-8"
	)
	(raw_dir / "context_articles.json").write_text(
		json.dumps(context_articles, ensure_ascii=False), encoding="utf-8"
	)
	return raw_dir


def test_convert_hotpotqa_raw_distractor_builds_case_local_graph(
	...
	layout = convert_hotpotqa_raw_dataset(
		raw_dir, tmp_path / "normalized-hotpot", variant="distractor"
	)
...
def test_convert_hotpotqa_raw_fullwiki_uses_supplied_graph(
	hotpotqa_raw_split_files, hotpotqa_fullwiki_files, tmp_path
):
...
def test_convert_musique_raw_preserves_support_start_and_path(
...
def test_convert_iirc_raw_flattens_nested_questions_and_context(
	iirc_raw_archive, tmp_path
):
	raw_layout = fetch_iirc_dataset(
		tmp_path / "iirc-raw", archive_url=iirc_raw_archive.as_uri()
	)
	layout = convert_iirc_raw_dataset(
		raw_layout.raw_dir / "iirc", tmp_path / "normalized-iirc"
	)
	...
	assert len({node.lower() for node in cases[0].gold_support_nodes}) == len(
		cases[0].gold_support_nodes
	)
...
def test_convert_iirc_raw_supports_html_context_articles_and_title_aliases(tmp_path):
	...
	assert (
		graph.links_between("Florida", "United States")[0].anchor_text
		== "United States"
	)
