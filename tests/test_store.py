...
def test_prepare_normalized_graph_store_accepts_graph_source_directory(iirc_questions, iirc_graph_records, tmp_path):
    ...
    shard_a.write_text(json.dumps(iirc_graph_records[0], ensure_ascii=False) + "\n", encoding="utf-8")
    shard_b.write_text(
        "\n".join(json.dumps(record, ensure_ascii=False) for record in iirc_graph_records[1:]) + "\n",
        encoding="utf-8",
    )
