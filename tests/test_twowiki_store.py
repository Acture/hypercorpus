...
def test_prepare_2wiki_store_writes_manifest_catalog_and_shards(
	prepared_two_wiki_store,
):
	...
	manifest_payload = json.loads(
		prepared_two_wiki_store.manifest_path.read_text(encoding="utf-8")
	)
...
def test_sharded_link_context_store_uses_catalog_before_loading_shards(
	prepared_two_wiki_store, tmp_path
):
	store = ShardedLinkContextStore(
		prepared_two_wiki_store.root, cache_dir=tmp_path / "cache"
	)
...
def test_sharded_link_context_store_remote_store_downloads_shard_once(
	prepared_two_wiki_store, tmp_path
):
	@dataclass
	...
	store = ShardedLinkContextStore(
		"s3://mock-bucket/2wiki-store", cache_dir=cache_dir, object_store=remote
	)
	...
	assert document is not None
	shard_key = next(
		iter(remote.downloads.keys() - {"manifest.json", "index/catalog.sqlite"})
	)
...
def test_prepare_2wiki_store_emits_phase_logs(two_wiki_archives, tmp_path, caplog):
	...
	assert any(
		"Phase 2/2: writing catalog and shards" in message for message in messages
	)


def test_prepare_2wiki_store_skips_duplicate_titles(
	tmp_path, two_wiki_questions, caplog
):
	...
	graph_path.write_text(
		"\n".join(
			json.dumps(record, ensure_ascii=False) for record in duplicate_records
		)
		+ "\n",
		encoding="utf-8",
	)
	for split in ("train", "dev", "test"):
		(questions_dir / f"{split}.json").write_text(
			json.dumps(two_wiki_questions, ensure_ascii=False), encoding="utf-8"
		)
	...
	messages = [record.getMessage() for record in caplog.records]
	assert any(
		"Skipped 1 duplicate 2Wiki node title" in message for message in messages
	)


def test_prepare_2wiki_store_rejects_existing_partial_output(
	...
	try:
		...
		raise AssertionError(
			"prepare_2wiki_store should reject an existing partial output directory"
		)
